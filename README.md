...
Works with `.ico` files. You can also use `.png`, `.jpg`, `.jpeg`, and `.webp` files; the app converts them to `.ico` automatically and saves the converted file next to the original.
...
3. Browse for an image file (`.ico`, `.png`, `.jpg`, `.jpeg`, or `.webp`).
...
If you pick a PNG, JPG, JPEG, or WebP file, the app creates a sibling `.ico` file (for example `Backup.png` becomes `Backup.ico` in the same folder) and uses that file for the drive icon. Large images are resized to fit Windows icon limits.
